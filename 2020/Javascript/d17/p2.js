...
function countActive(active, adj, pos, size) {
    ...
    for (let a of adj) {
        ...
        size.w.low = Math.min(size.w.low, nw);
        size.w.hi = Math.max(size.w.hi, nw);

        let apos = { x: nx, y: ny, z: nz , w: nw };
        let h = hash(apos);
        if (h in active) {
            count += 1;
        }
