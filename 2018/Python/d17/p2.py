...
def run_left(y, x, clay, visited):

    # run left until there is no water left
    while hash(y, x) in visited:
        x -= 1

    # if you hit clay return true
    return hash(y, x) in clay

def run_right(y, x, clay, visited):
    # run right until there is no water left
    while hash(y, x) in visited:
        x += 1
...
def run_down(y, x, clay, visited):
    # below you must be a secure guy or clay
    return (hash(y + 1, x) in clay) or (hash(y + 1, x) in retained)

def count_stable(clay, visited):
    total = 0
    lst = list(reversed(sorted(visited.values())))
    for y, x in lst:

        r1 = run_left(y, x, clay, visited)
        r2 = run_right(y, x, clay, visited)
        r3 = run_down(y, x, clay, visited)

        if r1 and r2 and r3:
            retained[hash(y,x)] = True
            total += 1
...
def sol():
    ...
    return count_stable(clay, visited)
