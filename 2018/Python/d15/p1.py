...
def damage(nr, nc, units, grid):
    for unit in units:
        if unit[R] == nr and unit[C] == nc:
            unit[H] -= 3 # do damage to unit
            if unit[H] <= 0:
...
def do_damage(possible, nr, nc, units, grid):
    ...
    return damage(lowest[R], lowest[C], units, grid)
    ...
    while True:
        ...
        for unit in units:
            ...
            if num_e == 0 or num_g == 0:
                return turns, sum(map(lambda x: x[H] if not x[D] else 0, units))
            ...
            if len(possible) > 0:
                ...
                result = do_damage(possible, nr, nc, units, grid)
                done = True
